...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="bash-completion python test"
...
src_prepare() {
	...
	# replace sensible-*
	find "${S}" -type f -exec sed -i -e "s/sensible-editor/vim/g" -e "s/sensible-pager/less/g" {} + || die
}
...
src_install() {
	...
	dosym "/usr/bin/debchange" "/usr/bin/dch"

	use bash-completion || rm -r "${D}/usr/share/bash-completion" || die
}
